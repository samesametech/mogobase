...
// Watermelon's change-tracking only fires for field setters generated by
// @field decorators. Our adapter writes the JSON blob directly to `_raw.data`
// to stay schemaless, so we manually flag the row as needing sync. A row
// that's still 'created' (never been pulled by the server) stays 'created' —
// only 'synced' transitions to 'updated'.
function markUpdated(raw: any) {
  if (raw._status === "created") return
  raw._status = "updated"
  const existing = typeof raw._changed === "string" && raw._changed.length > 0
    ? raw._changed.split(",")
    : []
  for (const col of ["data", "deleted_at"]) {
    if (!existing.includes(col)) existing.push(col)
  }
  raw._changed = existing.join(",")
}
