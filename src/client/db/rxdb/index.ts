...
export class MogobaseClientDB {
  ...
  // Backend-agnostic reactivity: the hooks call this instead of reaching into
  // ._rx.$ so WatermelonDB (or another backend) can plug in the same way.
  observeChanges(name: string): { subscribe: (fn: () => void) => { unsubscribe: () => void } } {
    const a = this.model(name)
    return {
      subscribe: (fn: () => void) => {
